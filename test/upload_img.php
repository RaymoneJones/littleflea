    ...
    if ($conn->connect_error){
        ...
        if ($number) {
            ...
            $sql_insert = "INSERT INTO tb_goods ".
                "(goods_name,userid,goods_img,goods_detail,goods_from, goods_price,goods_num) ".
                "VALUES ".
                "('$goods_name','$userid','$image','$goods_detail','$goods_from','$goods_price','$goods_num')";
            $res_insert=mysqli_query( $conn, $sql_insert );
            if ($res_insert) {
                echo "<script>alert('商品信息已提交，请等待管理员审核！\\n审核结果将在第一时间通过您预留邮箱发送，请注意查收 。'); </script>";
                echo '<script>window.location="goods_upload.php";</script>';
            } else {
                printf("Error: %s\n", mysqli_error($conn));
