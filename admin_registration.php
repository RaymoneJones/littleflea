...
<script>
    $('#matter_table1').bootstrapTable({
            url: 'data/a_r_data.php?table=per1',
            method: "post",
            dataType: "json",
            height: 500,
    ...
    $('#matter_table2').bootstrapTable({
            url: 'data/a_r_data.php?table=per2',
            method: "post",
            dataType: "json",
            height: 500,
            striped: true, //是否显示行间隔色
    ...
    function aFormatter1(value, row, index) {
        return [
            '<a class="btn btn-info btn-xs" href="revise/a_r_revise.php?no='+row.no+'&&username='+row.username+'&&email='+row.email+'&&caozuo0=1">通过</a>'
        ].join("")
    }
